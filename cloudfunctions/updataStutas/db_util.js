...
const dataUtil = require('./data_util.js');
const cloud = require('wx-server-sdk')
// hesuanofficial-8gp0jtoi3d51cbd9
//cloud1-5gde1hoac3a7d47f
cloud.init({
...
async function insertBatch(collectionName, data, size = 1000) {
	...
	//return query._id;没有返回，作者设的注释，没有修改
}
...
async function edit(collectionName, where, data) {
	...
	if (util.isDefined(where)) {
		if (typeof (where) == 'string' || typeof (where) == 'number')//如果查询条件是字符串或者数字 说明是id
			query = await query.doc(where);//用id查询
		else
			query = await query.where(fmtWhere(where));//不是就用条件查询
	}
 ...
 */
async function getOne(collectionName, where, fields = '*', orderBy = {}) {//1, golbal.PID
	// 根据ID查询还是根据条件查询
	if (typeof (where) == 'string' || typeof (where) == 'number') {
		where = {
			_id: where
	...
	// 查询条件 
	let query = await db.collection(collectionName)
		.where(fmtWhere(where))//查询条件 where里面写什么
		.limit(1);//限制一条

	// 取出特定字段 
	if (fields != '*')
		query = await query.field(fmtFields(fields));//把选择的字段变成true
...
/**
 * where处理 单表 可以把二维数组展开和一维数组的条件展开
...
function fmtWhere(where) {//用到递归
	if (util.isDefined(where.and) || util.isDefined(where.or)) {//没有数组类型的命令就跳过
		let whereEx = null;
