...
class BaseModel extends Model {
	...
	static _getWhere(where, mustPID = true) { 
		if (mustPID) {
			if (typeof (where) == 'string' || typeof (where) == 'number') {//如果where、是id
				where = {
					_id: where,
	 ...
	 *///join_data//EXPORT_CLOUD_ID,EXPORT_EDIT_TIME
	static async getOne(where, fields = '*', orderBy = {}, mustPID = true) {
		where = BaseModel._getWhere(where, mustPID); // 1, golbal.PID
	...
	static async insert(data, mustPID = true) {
        if (mustPID) data._pid = BaseModel._getProjectId();//A00、
