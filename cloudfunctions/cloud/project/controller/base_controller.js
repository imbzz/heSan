...
class BaseController extends Controller {//control只有四个变量:路由 用户身份 所以参数 数据参数

	constructor(route, openId, event) {
		super(route, openId, event);

		if (config.TEST_MODE)//调试相关：false 所以不执行
			openId = config.TEST_TOKEN_ID;//TEST_TOKEN_ID为空

		if (!openId) {//用户id空时 抛出错误
			console.error('OPENID is unfined');
			throw new AppError('OPENID is unfined', appCode.SVR);
		}

		// 模板判定
		if (config.PID) {//PID==A00 非空
			global.PID = config.PID;//上方定义:global.PID = 'unknown'
		} else {
			if (event.PID) global.PID = event.PID;//管理者设置预约时event.PID上传上来的数据就是A00
		}
	 ...
	 */
	validateData(rules = {}) {
		let input = this._request;//_request为event.params事件参数
