...
class Model {
	...
	static async getOne(where, fields = '*', orderBy = {}) {
        return await dbUtil.getOne(this.CL, where, fields, orderBy);//this.CL指向构造的那个对象，不是base，也不是modelbas，此函数为static函数,为全局函数
	}
	...
	static async insert(data) {
		// 自动ID(true)
		if (this.ADD_ID) {//this.FIELD_PREFIX的值为:MEET_ID
        ...
        console.log(data);
        let result=await dbUtil.insert(this.CL, data);
		return result;
	}
